...
class ChangeWordViewController: NewWordViewController {
    ...
    override func initialSetting() {
        let v = gb.buildEditView(categories: categories)
        v.tag = 12345
        view.addSubview(v)
        setView()
    }
    ...
    override func setView() {
        ...
        category_label.text = words[0].category
    ...
    override func submit(_ sender: Any) {
        ...
        ref.child("words").child(String(words[0].db_index)).child("category").setValue(category_label.text!)
