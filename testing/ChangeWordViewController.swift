...
class ChangeWordViewController: NewWordViewController {
    ...
    override func initialSetting() {
        let v = gb.buildEditView(categories: default_categories)
        v.tag = 12345
        view.addSubview(v)
        setView()
    }
    ...
    override func setView() {
        super.setView()
        ed_text_english.text = words[current-1].english
        ed_text_russian.text = words[current-1].russian
        category_label.text = words[current-1].category
    ...
    override func submit(_ sender: Any) {
        ...
        ref.child("words").child(String(words[current-1].db_index)).child("English").setValue(eng)
        ref.child("words").child(String(words[current-1].db_index)).child("Russian").setValue(rus)
        ref.child("words").child(String(words[current-1].db_index)).child("category").setValue(category_label.text!)
